...
def bring_matrix(our_brings, their_brings, world, on_cell=None, **kw):
    ...
    for i, ours in enumerate(our_brings):
        ...
        for j, theirs in enumerate(their_brings):
            est = matchup_win_prob(ours, theirs, world, **kw)
            if on_cell:
                on_cell(i, j, est)
            row.append(est)
        rows.append(row)
...
def aggregate(matrix, iters=3000):
    ...
    A = [[c.p if isinstance(c, Estimate) else float(c) for c in row]
         for row in matrix]
    if not A or not A[0]:
...
def evaluate(our_brings, their_brings, world, on_cell=None, iters=3000, **kw):
    ...
    """
    matrix = bring_matrix(our_brings, their_brings, world, on_cell=on_cell, **kw)
